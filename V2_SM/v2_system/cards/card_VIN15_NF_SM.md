# Evidence Card — VIN15_NF_SM

| Field | Value |
|---|---|
| VIN | `VIN15_NF_SM` |
| Tier (production prob) | **[GREEN]** |
| Production probability | `0.0640` |
| OOF probability (honest) | `0.2542` ([GREEN]) |
| Failed | `No` |
...
|---|---|---|---|---|---|---|---|
| 1 | `vsi_withinwk_std_ratio_30d_w` | `0.9414` | `38.2`th | `-0.478` | `-0.4237` | protective | within-week voltage noise is 0.94x own baseline — low 38th fleet percentile; z=-0.48 |
| 2 | `rest_vsi_p05_delta90` | `0.5713` | `94.1`th | `+0.934` | `-0.2526` | protective | rest-voltage floor delta = +0.571 V vs own baseline — stable/rising; z=+0.93, 94th fleet pctile |
...
| 4 | `dip_depth_last90_delta` | `0.2456` | `67.6`th | `-0.029` | `-0.0041` | protective | crank dip depth delta = +0.246 V vs baseline — deepening dips (heavier load signature); z=-0.03, 68th fleet pctile |
...
| First channel / first fire date | `A1_crank_burst` / `2025-02-08` | — |
...
Reducing vsi_range_trend by 0.064 units (from 0.000 to -0.064) would move production prob below 0.35 (GREEN threshold), all else equal. [delta_z = -0.989]
