# Evidence Card — VIN5_NF_SM

| Field | Value |
|---|---|
| VIN | `VIN5_NF_SM` |
| Tier (production prob) | **[RED]** |
| Production probability | `0.8722` |
| OOF probability (honest) | `0.9575` ([RED]) |
| Failed | `No` |
| Priority | **P0** (persistent RED 6 weeks) |
| Badges | `WATCHLIST` |
...
|---|---|---|---|---|---|---|---|
| 1 | `rest_vsi_p05_delta90` | `-1.1896` | `14.7`th | `-1.182` | `+0.3195` | toward failure | rest-voltage floor delta = -1.190 V vs own baseline — strong sagging (battery cascade risk); z=-1.18, 15th fleet pctile |
...
| 3 | `vsi_withinwk_std_ratio_30d_w` | `1.1066` | `61.8`th | `-0.209` | `-0.1850` | protective | within-week voltage noise is 1.11x own baseline — elevated 62th fleet percentile; z=-0.21 |
| 4 | `dip_depth_last90_delta` | `0.9270` | `85.3`th | `+0.698` | `+0.0983` | toward failure | crank dip depth delta = +0.927 V vs baseline — deepening dips (heavier load signature); z=+0.70, 85th fleet pctile |
...
| Persistence (RED >=3wk) | `True` (terminal); streak=`6` wk | Validated FP: 4/20 NF end in persistence fire state |
| First channel / first fire date | `A1_crank_burst` / `2025-03-25` | — |
...
Reducing rest_vsi_p05_delta90 by 1.551 units (from -1.190 to 0.361) would move production prob below 0.35 (GREEN threshold), all else equal. [delta_z = +1.863]
