# Evidence Card — VIN16_NF_SM

| Field | Value |
|---|---|
| VIN | `VIN16_NF_SM` |
| Tier (production prob) | **[GREEN]** |
| Production probability | `0.0102` |
| OOF probability (honest) | `0.0433` ([GREEN]) |
| Failed | `No` |
...
|---|---|---|---|---|---|---|---|
| 1 | `vsi_withinwk_std_ratio_30d_w` | `0.5518` | `5.9`th | `-1.113` | `-0.9867` | protective | within-week voltage noise is 0.55x own baseline — low 6th fleet percentile; z=-1.11 |
...
| 3 | `rest_vsi_p05_delta90` | `0.1055` | `61.8`th | `+0.375` | `-0.1013` | protective | rest-voltage floor delta = +0.105 V vs own baseline — stable/rising; z=+0.37, 62th fleet pctile |
| 4 | `dip_depth_last90_delta` | `0.4834` | `73.5`th | `+0.225` | `+0.0317` | toward failure | crank dip depth delta = +0.483 V vs baseline — deepening dips (heavier load signature); z=+0.22, 74th fleet pctile |
...
| First channel / first fire date | `A1_crank_burst` / `2025-09-07` | — |
...
Changing vsi_range_trend by 0.123 units (to -0.123) would theoretically cross the GREEN threshold, but this value is outside plausible fleet range [-0.080, 0.202]. Multiple drivers are concurrently elevated; single-feature counterfactual is not achievable.
