# Evidence Card — VIN20_NF_SM

| Field | Value |
|---|---|
| VIN | `VIN20_NF_SM` |
| Tier (production prob) | **[RED]** |
| Production probability | `0.5652` |
| OOF probability (honest) | `0.6228` ([RED]) |
| Failed | `No` |
...
|---|---|---|---|---|---|---|---|
| 1 | `vsi_range_trend` | `0.0000` | `39.7`th | `-0.454` | `+0.1877` | toward failure | drive-voltage range trend = +0.0000 V/wk — flat/narrowing; z=-0.45, 40th fleet pctile; NOTE: multivariate suppressor (see coefficient table) |
| 2 | `rest_vsi_p05_delta90` | `0.1829` | `64.7`th | `+0.468` | `-0.1264` | protective | rest-voltage floor delta = +0.183 V vs own baseline — stable/rising; z=+0.47, 65th fleet pctile |
| 3 | `vsi_withinwk_std_ratio_30d_w` | `1.2614` | `76.5`th | `+0.044` | `+0.0387` | toward failure | within-week voltage noise is 1.26x own baseline — elevated 76th fleet percentile; z=+0.04 |
...
| Persistence (RED >=3wk) | `False` (terminal); streak=`1` wk | Validated FP: 4/20 NF end in persistence fire state |
...
- **Evidence state:** `RED_tier_no_channel_yet`
- **Empirical n:** 10 (retrospective fleet)
- **Median lead to failure:** 206 days
- **95% bootstrap CI:** [128d, 273d]
- **Scheduling window:** 30–60 days or next scheduled service
- **Honest caveat:** Act on channel fire, not score alone. 2 RED NF are false positives (VIN5_NF, VIN20_NF).
- **NOT a countdown clock.** This window is retrospective population data, not a per-truck timer.

## (f) Counterfactual

Reducing vsi_range_trend by 0.027 units (from 0.000 to 0.027) would move production prob below 0.35 (GREEN threshold), all else equal. [delta_z = +0.422]
