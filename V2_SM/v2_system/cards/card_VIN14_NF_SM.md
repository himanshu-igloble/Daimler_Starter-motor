# Evidence Card — VIN14_NF_SM

| Field | Value |
|---|---|
| VIN | `VIN14_NF_SM` |
| Tier (production prob) | **[GREEN]** |
| Production probability | `0.0039` |
| OOF probability (honest) | `0.0412` ([GREEN]) |
| Failed | `No` |
...
|---|---|---|---|---|---|---|---|
| 1 | `vsi_withinwk_std_ratio_30d_w` | `0.4511` | `2.9`th | `-1.278` | `-1.1322` | protective | within-week voltage noise is 0.45x own baseline — low 3th fleet percentile; z=-1.28 |
| 2 | `rest_vsi_p05_delta90` | `0.4257` | `82.4`th | `+0.759` | `-0.2053` | protective | rest-voltage floor delta = +0.426 V vs own baseline — stable/rising; z=+0.76, 82th fleet pctile |
...
| 4 | `dip_depth_last90_delta` | `0.8605` | `82.4`th | `+0.627` | `+0.0883` | toward failure | crank dip depth delta = +0.861 V vs baseline — deepening dips (heavier load signature); z=+0.63, 82th fleet pctile |
...
Changing vsi_range_trend by 0.154 units (to -0.154) would theoretically cross the GREEN threshold, but this value is outside plausible fleet range [-0.080, 0.202]. Multiple drivers are concurrently elevated; single-feature counterfactual is not achievable.
