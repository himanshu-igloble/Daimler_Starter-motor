---
# Work Order: VIN3_F_SM
**Priority:** P0 | **Tier:** GREEN | **Trigger:** A2_battery_cascade_fired
**Date:** 2026-06-12
...
**Evidence summary:** A2_battery_cascade(fired); persistence_terminal(active)

**Model probability:** 0.313 (tier: GREEN)
**Physics mode:** Battery voltage floor sagging (rest VSI p05 decline) triggering cascade load on starter; battery-primary mode

### Top-3 Drivers
1. **vsi_range_trend** (protective)  
   drive-voltage range trend = +0.1196 V/wk — widening (risk direction univariately); z=+1.39, 91th fleet pctile; NOTE: multivariate suppressor (see coefficient table)  
   Contribution: -0.573 std | z=1.39 | fleet p91
2. **rest_vsi_p05_delta90** (toward failure)  
   rest-voltage floor delta = -1.251 V vs own baseline — strong sagging (battery cascade risk); z=-1.26, 12th fleet pctile  
   Contribution: +0.339 std | z=-1.25 | fleet p12
3. **vsi_withinwk_std_ratio_30d_w** (toward failure)  
   within-week voltage noise is 1.37x own baseline — elevated 79th fleet percentile; z=+0.22  
   Contribution: +0.192 std | z=0.22 | fleet p79

### Channel History
- First channel: **persistence** @ 2025-06-30  
- Persistent RED streak: 0 weeks  
