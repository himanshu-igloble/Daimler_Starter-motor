...
**Evidence summary:** H1_momentum(Δprob=+0.595); persistence_terminal(active)

**Model probability:** 0.914 (tier: RED)
...
1. **rest_vsi_p05_delta90** (toward failure)  
   rest-voltage floor delta = -1.657 V vs own baseline — strong sagging (battery cascade risk); z=-1.74, 6th fleet pctile  
   Contribution: +0.471 std | z=-1.74 | fleet p6
2. **dip_depth_last90_delta** (toward failure)  
   crank dip depth delta = +1.577 V vs baseline — deepening dips (heavier load signature); z=+1.39, 97th fleet pctile  
   Contribution: +0.196 std | z=1.39 | fleet p97
3. **vsi_range_trend** (toward failure)  
...
### Channel History
- First channel: **persistence** @ 2025-09-22  
